...
const SUPABASE_ANON_KEY=CFG.supabasePublishableKey||CFG.supabaseAnonKey||'';
...
async function loginWithPassword(){
  ...
  if(error){
    ...
    if(/invalid login credentials/i.test(msg)){
      setAuthMessage('Password salah, atau user maheshapanca@gmail.com belum dibuat di Supabase Authentication.','err');
    }else if(/email not confirmed/i.test(msg)){
      setAuthMessage('Email user Supabase belum dikonfirmasi. Aktifkan Auto Confirm saat membuat user.','err');
    }else{
      setAuthMessage(msg||'Login gagal.','err');
    }
    return
  }
